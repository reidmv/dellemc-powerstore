...
class PowerstoreVolumeGroupRestoreTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/volume_group/%{id}/restore'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('from_snap_id', 'body', 'from_snap_id', 'from_snap_id'),
  ...
  if __FILE__ == $0
    PowerstoreVolumeGroupRestoreTask.run
  end
