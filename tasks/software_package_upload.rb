...
class PowerstoreSoftwarePackageUploadTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/software_package'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('upload_file', 'formData', 'upload_file', 'upload_file'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreSoftwarePackageUploadTask.run
  end
