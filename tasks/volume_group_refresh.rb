...
class PowerstoreVolumeGroupRefreshTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/volume_group/%{id}/refresh'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('backup_snap_profile', 'body', 'backup_snap_profile', 'backup_snap_profile'),
      op_param('create_backup_snap', 'body', 'create_backup_snap', 'create_backup_snap'),
      op_param('from_object_id', 'body', 'from_object_id', 'from_object_id'),
  ...
  if __FILE__ == $0
    PowerstoreVolumeGroupRefreshTask.run
  end
