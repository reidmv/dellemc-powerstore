...
class PowerstoreStorageContainerMountTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/storage_container/%{id}/mount'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('datastore_name', 'body', 'datastore_name', 'datastore_name'),
  ...
  if __FILE__ == $0
    PowerstoreStorageContainerMountTask.run
  end
