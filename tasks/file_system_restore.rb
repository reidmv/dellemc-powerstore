...
class PowerstoreFileSystemRestoreTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/file_system/%{id}/restore'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('copy_name', 'body', 'copy_name', 'copy_name'),
  ...
  if __FILE__ == $0
    PowerstoreFileSystemRestoreTask.run
  end
