...
class PowerstoreIpPortModifyTask < TaskHelper
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('add_current_usages', 'body', 'add_current_usages', 'add_current_usages'),
  ...
  if __FILE__ == $0
    PowerstoreIpPortModifyTask.run
  end
