...
class PowerstoreAlertModifyTask < TaskHelper
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('acknowledged_severity', 'body', 'acknowledged_severity', 'acknowledged_severity'),
      op_param('id', 'path', 'id', 'id'),
      op_param('is_acknowledged', 'body', 'is_acknowledged', 'is_acknowledged'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
