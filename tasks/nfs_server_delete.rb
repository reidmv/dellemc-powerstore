...
class PowerstoreNfsServerDeleteTask < TaskHelper
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('is_skip_unjoin', 'body', 'is_skip_unjoin', 'is_skip_unjoin'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreNfsServerDeleteTask.run
  end
