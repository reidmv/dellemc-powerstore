...
class PowerstoreSmtpConfigModifyTask < TaskHelper
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('port', 'body', 'port', 'port'),
      op_param('source_email', 'body', 'source_email', 'source_email'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreSmtpConfigModifyTask.run
  end
