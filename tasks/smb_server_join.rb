...
class PowerstoreSmbServerJoinTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/smb_server/%{id}/join'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('organizational_unit', 'body', 'organizational_unit', 'organizational_unit'),
      op_param('reuse_computer_account', 'body', 'reuse_computer_account', 'reuse_computer_account'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreSmbServerJoinTask.run
  end
