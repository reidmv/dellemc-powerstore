...
class PowerstoreVolumeSnapshotTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/volume/%{id}/snapshot'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('creator_type', 'body', 'creator_type', 'creator_type'),
  ...
  if __FILE__ == $0
    PowerstoreVolumeSnapshotTask.run
  end
