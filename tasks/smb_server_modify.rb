...
class PowerstoreSmbServerModifyTask < TaskHelper
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('computer_name', 'body', 'computer_name', 'computer_name'),
      op_param('description', 'body', 'description', 'description'),
      op_param('domain', 'body', 'domain', 'domain'),
      op_param('id', 'path', 'id', 'id'),
      op_param('is_standalone', 'body', 'is_standalone', 'is_standalone'),
      op_param('local_admin_password', 'body', 'local_admin_password', 'local_admin_password'),
      op_param('netbios_name', 'body', 'netbios_name', 'netbios_name'),
      op_param('workgroup', 'body', 'workgroup', 'workgroup'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreSmbServerModifyTask.run
  end
