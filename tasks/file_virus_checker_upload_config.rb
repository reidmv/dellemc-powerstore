...
class PowerstoreFileVirusCheckerUploadConfigTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/file_virus_checker/%{id}/upload_config'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('body', 'formData', 'body', 'body'),
  ...
  if __FILE__ == $0
    PowerstoreFileVirusCheckerUploadConfigTask.run
  end
