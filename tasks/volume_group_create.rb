...
class PowerstoreVolumeGroupCreateTask < TaskHelper
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('volume_ids', 'body', 'volume_ids', 'volume_ids'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreVolumeGroupCreateTask.run
  end
