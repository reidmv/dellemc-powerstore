...
class PowerstoreClusterModifyclusterpropertiesTask < TaskHelper
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('physical_mtu', 'body', 'physical_mtu', 'physical_mtu'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreClusterModifyclusterpropertiesTask.run
  end
