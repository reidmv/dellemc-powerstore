...
class PowerstoreReplicationSessionFailoverTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/replication_session/%{id}/failover'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('is_planned', 'body', 'is_planned', 'is_planned'),
      op_param('reverse', 'body', 'reverse', 'reverse'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreReplicationSessionFailoverTask.run
  end
