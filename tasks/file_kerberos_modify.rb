...
class PowerstoreFileKerberosModifyTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_path = '/file_kerberos/%{id}'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('add_kdc_addresses', 'body', 'add_kdc_addresses', 'add_kdc_addresses'),
      op_param('id', 'path', 'id', 'id'),
      op_param('kdc_addresses', 'body', 'kdc_addresses', 'kdc_addresses'),
      op_param('port_number', 'body', 'port_number', 'port_number'),
      op_param('realm', 'body', 'realm', 'realm'),
      op_param('remove_kdc_addresses', 'body', 'remove_kdc_addresses', 'remove_kdc_addresses'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreFileKerberosModifyTask.run
  end
