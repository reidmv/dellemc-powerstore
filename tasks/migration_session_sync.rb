...
class PowerstoreMigrationSessionSyncTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/migration_session/%{id}/sync'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('rescan_complete', 'body', 'rescan_complete', 'rescan_complete'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreMigrationSessionSyncTask.run
  end
