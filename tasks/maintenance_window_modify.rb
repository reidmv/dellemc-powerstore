...
class PowerstoreMaintenanceWindowModifyTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_path = '/maintenance_window/%{id}'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('end_offset', 'body', 'end_offset', 'end_offset'),
  ...
  if __FILE__ == $0
    PowerstoreMaintenanceWindowModifyTask.run
  end
