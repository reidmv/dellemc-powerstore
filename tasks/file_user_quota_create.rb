...
class PowerstoreFileUserQuotaCreateTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/file_user_quota'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('tree_quota_id', 'body', 'tree_quota_id', 'tree_quota_id'),
      op_param('uid', 'body', 'uid', 'uid'),
      op_param('unix_name', 'body', 'unix_name', 'unix_name'),
      op_param('windows_name', 'body', 'windows_name', 'windows_name'),
      op_param('windows_sid', 'body', 'windows_sid', 'windows_sid'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreFileUserQuotaCreateTask.run
  end
