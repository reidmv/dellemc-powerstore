...
class PowerstoreApplianceTimeToFullTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/appliance/%{id}/time_to_full'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('metric_type', 'body', 'metric_type', 'metric_type'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreApplianceTimeToFullTask.run
  end
