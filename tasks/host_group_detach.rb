...
class PowerstoreHostGroupDetachTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/host_group/%{id}/detach'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('volume_id', 'body', 'volume_id', 'volume_id'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreHostGroupDetachTask.run
  end
