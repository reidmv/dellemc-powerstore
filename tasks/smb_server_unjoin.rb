...
class PowerstoreSmbServerUnjoinTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/smb_server/%{id}/unjoin'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('is_skip_AD_unjoin', 'body', 'is_skip_ad_unjoin', 'is_skip_ad_unjoin'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreSmbServerUnjoinTask.run
  end
