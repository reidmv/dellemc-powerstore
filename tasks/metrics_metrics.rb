...
class PowerstoreMetricsMetricsTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/metrics/generate'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('entity', 'body', 'entity', 'entity'),
      op_param('entity_id', 'body', 'entity_id', 'entity_id'),
  ...
  if __FILE__ == $0
    PowerstoreMetricsMetricsTask.run
  end
