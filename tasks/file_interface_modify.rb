...
class PowerstoreFileInterfaceModifyTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_path = '/file_interface/%{id}'
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('ip_address', 'body', 'ip_address', 'ip_address'),
      op_param('is_disabled', 'body', 'is_disabled', 'is_disabled'),
  ...
  if __FILE__ == $0
    PowerstoreFileInterfaceModifyTask.run
  end
