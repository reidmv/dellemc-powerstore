...
class PowerstoreMigrationRecommendationCreateTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/migration_recommendation'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('evacuate_appliance', 'body', 'evacuate_appliance', 'evacuate_appliance'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreMigrationRecommendationCreateTask.run
  end
