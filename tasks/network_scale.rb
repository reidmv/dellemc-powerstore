...
class PowerstoreNetworkScaleTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/network/%{id}/scale'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('add_port_ids', 'body', 'add_port_ids', 'add_port_ids'),
      op_param('force', 'body', 'force', 'force'),
      op_param('id', 'path', 'id', 'id'),
      op_param('remove_port_ids', 'body', 'remove_port_ids', 'remove_port_ids'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreNetworkScaleTask.run
  end
