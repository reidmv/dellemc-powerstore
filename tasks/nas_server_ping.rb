...
class PowerstoreNasServerPingTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Post'
    operation_path = '/nas_server/%{id}/ping'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('destination_address', 'body', 'destination_address', 'destination_address'),
      op_param('id', 'path', 'id', 'id'),
      op_param('is_ipv6', 'body', 'is_ipv6', 'is_ipv6'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreNasServerPingTask.run
  end
