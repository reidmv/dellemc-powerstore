...
class PowerstoreSnapshotRuleDeleteTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_path = '/snapshot_rule/%{id}'
  ...
  def format_params(key_values)
    ...
    op_params = [
      op_param('delete_snaps', 'body', 'delete_snaps', 'delete_snaps'),
  ...
  if __FILE__ == $0
    PowerstoreSnapshotRuleDeleteTask.run
  end
