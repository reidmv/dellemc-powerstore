...
class PowerstoreEthPortModifyTask < TaskHelper
  ...
  def format_params(key_values)
    ...
    op_params = [
      ...
      op_param('requested_speed', 'body', 'requested_speed', 'requested_speed'),
      ]
    op_params.each do |i|
      location = i[:location]
      name     = i[:name]
  ...
  if __FILE__ == $0
    PowerstoreEthPortModifyTask.run
  end
